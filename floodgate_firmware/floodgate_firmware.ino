...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (doc.containsKey("source") && strcmp(doc["source"], "openweather_api") == 0) {
    ...
    if (highRisk == 1) {
      sleepDuration = HIGH_RISK_SLEEP_TIME; // 30 seconds
      Serial.println("\n[RISK ALERT] High Risk Weather Forecast Detected!");
      Serial.printf("[RISK ALERT] Updated Sleep Duration to %d seconds (30 sec).\n", sleepDuration);
    } else {
      sleepDuration = DEFAULT_SLEEP_TIME;   // 1 minute
      Serial.println("\n[RISK ALERT] Low/Normal Risk Weather. Setting Sleep Duration to 1 min.");
    }
  }
}
